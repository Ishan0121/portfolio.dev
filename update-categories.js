...
const categoriesCode = `const modelCategories = [
  {
    name: "Cyber / Tech",
    items: [
...
const models = modelCategories.flatMap(c => c.items);`;

clientFile = clientFile.replace(/const models = \[\s*\/\/ Cyber \/ Tech[\s\S]*?\];/m, categoriesCode);

clientFile = clientFile.replace(
  /const \[isDropdownOpen, setIsDropdownOpen\] = useState\(false\);/,
  `const [isDropdownOpen, setIsDropdownOpen] = useState(false);\n  const [expandedCategory, setExpandedCategory] = useState("Fantasy & Abstract");`
);

const dropdownMenuCode = `{modelCategories.map((category, idx) => (
                    <div key={idx} className="mb-1 last:mb-0">
                      ...
                      >
                        <h3 className="text-[10px] sm:text-xs font-mono tracking-widest uppercase text-foreground/80 font-bold">
                          {category.name}
                        </h3>
                        <Icon 
                          icon="lucide:chevron-down" 
                          className={\`w-3.5 h-3.5 transition-transform duration-300 \${expandedCategory === category.name ? 'rotate-180 text-primary' : 'text-muted-foreground'}\`} 
                        />
                      </button>
                          ...
                          >
                            <div className="grid grid-cols-1 gap-1 pt-1 pb-2 px-1">
                              {category.items.map(model => (
                                ...
                                >
                                  <div className={\`p-1.5 sm:p-2 rounded-md \${activeModel === model.id ? 'bg-primary/20' : 'bg-background'}\`}>
                                    {model.icon}
                                  </div>
...
clientFile = clientFile.replace(/\{models\.map\(model => \([\s\S]*?\}\)/m, dropdownMenuCode);
